...
function init(){
    ...
    p.View.TV.Channel = p.View.Base.extend({
        ...
        switchChannel: function(channel){
            ...
            if (this.channel === "porn" && !p.user.id) return;
        ...
        channelReload: function(response){
            if (response.items.length < 3) return;

            this.items = response.items.shuffle();
            this.currentIndex = 0;

            if (this.currentIndex > this.items.length - 2) this.loadChannel(this.channelReload.bind(this));
        },
        ...
        nextVideo: function(){
            if (this.currentIndex > this.items.length - 10) this.loadChannel(this.channelReload.bind(this));
        ...
        createVideoElement: function(src, loadedCallback){
            ...
            if (loadedCallback) $vid[0].oncanplaythrough = loadedCallback;
        ...
        resize: function(){
            if (this.$currentVideo && this.$currentVideo[0].videoWidth){
                ...
                var sh = this.$videoContainer.height() - ph;
                var vw = this.$currentVideo[0].videoWidth;
                var vh = this.$currentVideo[0].videoHeight;
                var sa = sw / sh;
                var va = vw / vh;
                var nw = 0;
                var nh = 0;
        ...
        visibilityChange: function(){
            if (!this.$currentVideo || !this.$currentVideo[0].videoWidth) return;
        ...
        remove: function(){
            ...
            if (this.visibilityChangeBound) document.removeEventListener("visibilitychange", this.visibilityChangeBound, false);
        ...
        imageLoaded: function(){
            this.loadQueue--;
            if (this.loadQueue === 0) this.start();
        },
...
$(document).ready(function(){
    ...
    $("head").append(`<style type="text/css">
        ...
        div.tv-video-container video,
        div.tv-video-container canvas {
            display: block;
            position: absolute;
            margin: auto;
        }
